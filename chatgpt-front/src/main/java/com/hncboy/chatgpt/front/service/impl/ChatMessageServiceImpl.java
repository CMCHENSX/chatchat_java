...
import com.hncboy.chatgpt.front.handler.bizchain.*;
...
public class ChatMessageServiceImpl extends ServiceImpl<ChatMessageMapper, ChatMessageDO> implements ChatMessageService {
    ...
    @Resource
    private BizChainRouter bizChainRouter;

    @Resource
    private ChatRoomService chatRoomService;

    @Override
    public ResponseBodyEmitter sendMessage(ChatCompletion ChatCompletion, ResponseBodyEmitter emitter, HttpServletResponse response) {
        // 超时时间设置 3 分钟
//        SseEmitter emitter = new SseEmitter(3 * 60 * 1000L);
        emitter.onCompletion(() -> log.debug("请求参数：{}，Front-end closed the emitter connection.", ObjectMapperUtil.toJson(ChatCompletion)));
        emitter.onTimeout(() -> log.error("请求参数：{}，Back-end closed the emitter connection.", ObjectMapperUtil.toJson(ChatCompletion)));
...
//        ipRateLimiterEmitterChain.doChain(ChatCompletion, emitter, response);
        bizChainRouter.route(new HandlerContext<>(ChatCompletion,emitter, response, RequestType.CHAT_GPT));
        return emitter;
    }

    @Transactional(rollbackFor = Exception.class)
    @Override
    public ChatMessageDO initChatMessage(ChatCompletion ChatCompletion, ApiTypeEnum apiTypeEnum) {
...
//        chatMessageDO.setUserId(FrontUserUtil.getUserId());
        chatMessageDO.setContent(getPrompt(ChatCompletion));
