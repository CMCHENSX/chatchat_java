...
import com.hncboy.chatgpt.front.api.listener.SseEmitterStreamListener;
...
public class ApiKeyResponseEmitter implements ResponseEmitter {
    ...
    @Override
    public void requestToResponseEmitter(ChatCompletion ChatCompletion, ResponseBodyEmitter emitter, HttpServletResponse response) {
        // 初始化聊天消息
        ChatMessageDO chatMessageDO = chatMessageService.initChatMessage(ChatCompletion, ApiTypeEnum.API_KEY);
...
//        }
        ...
        // 获取 包含上下文 的 token 数量
        int totalTokenCount = (int) ChatCompletion.tokens();//TikTokensUtil.tokens(ChatCompletion.getModel(), ChatCompletion.getMessages());
        // 设置 promptTokens
        chatMessageDO.setPromptTokens(totalTokenCount);

        // 检查 tokenCount 是否超出当前模型的 Token 数量限制
        String exceedModelTokenLimitMsg = exceedModelTokenLimit(ChatCompletion, chatMessageDO.getModelName(), totalTokenCount, emitter);
...
//                .build();
        ...
        // 构建事件监听器
        ParsedEventSourceListener.Builder builder = new ParsedEventSourceListener.Builder()
...
//                .build();
        if (emitter instanceof SseEmitter){
            builder.addListener(new SseEmitterStreamListener(emitter));
        } else if (emitter instanceof ResponseBodyEmitter){
            builder.addListener(new ResponseBodyEmitterStreamListener(emitter));
        }
        ApiKeyChatClientBuilder.buildOpenAiStreamClient().streamChatCompletion(ChatCompletion, builder.build());
    }

    /**
     * 检查上下文消息的 Token 数是否超出模型限制
     *
     * @param ChatCompletion 对话请求
     * @param modelName          当前使用的模型名称
     * @param tokenCount         当前上下的总 Token 数量
     * @param emitter            SseEmitter
     */
    private String exceedModelTokenLimit(ChatCompletion ChatCompletion, String modelName, int tokenCount, ResponseBodyEmitter emitter) {
        // 当前模型最大 tokens
...
//            int currentPromptTokens = TikTokensUtil.tokens(modelName, ChatCompletion.getPrompt());
//            // 判断历史上下文是否超过限制
...
//        }
        ...
        try {
            emitter.send(ChatReplyMessageVO.reply(ChatCompletion.getModel(), msg));
        } catch (IOException e) {
