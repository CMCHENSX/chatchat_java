...
public class AccessTokenResponseEmitter implements ResponseEmitter {
    ...
    public void requestToResponseEmitter(ChatCompletion ChatCompletion, ResponseBodyEmitter emitter, HttpServletResponse response) {
        ...
        // 初始化聊天消息
        ChatMessageDO chatMessageDO = chatMessageService.initChatMessage(ChatCompletion, ApiTypeEnum.ACCESS_TOKEN);
...
//                .addListener(new ConsoleStreamListener())
                .addListener(new SseEmitterStreamListener(emitter))
                .setParser(parser)
