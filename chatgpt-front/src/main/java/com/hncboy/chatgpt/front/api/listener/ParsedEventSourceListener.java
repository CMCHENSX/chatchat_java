...
//        }
//        System.out.println(System.currentTimeMillis() + ":" + originalData);
        // 遍历监听器
        for (AbstractStreamListener listener : listeners) {
            if (isEnd) {
                listener.onComplete(originalData);
            } else {
                listener.onMessage(originalData, receivedMessage, chatReplyMessageVO, currentStreamMessageCount);
            }
        }
    }
    ...
    public void onFailure(@NotNull EventSource eventSource, @Nullable Throwable t, @Nullable Response response) {
        ...
        // 遍历监听器发送异常消息
        for (AbstractStreamListener listener : listeners) {
            listener.onError(responseStr, t, response);
        }
