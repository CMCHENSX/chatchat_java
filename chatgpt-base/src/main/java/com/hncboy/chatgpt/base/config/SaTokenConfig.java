...
//                    // 非管理端接口都必须 front 用户登录
                ...
                // 放行用户端邮箱注册
                .excludePathPatterns("/api/user/register/email")
                // 放行用户端图形验证码
                .excludePathPatterns("/api/user/get_pic_code")
                // 放行用户端邮箱登录
                .excludePathPatterns("/api/user/login/email")
                // swagger 放行
                .excludePathPatterns("/swagger-ui/**")
                .excludePathPatterns("/v3/api-docs/**")
                .excludePathPatterns("/api/chat_message/test");
    }
